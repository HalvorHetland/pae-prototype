...
export function ImageSprite({
  ...
  kenBurns?: boolean; kenBurnsScale?: number;
  radius?: number; fit?: string;
  ...
  const content = placeholder ? (
    <div style={{
      ...
      color: "#6b6458", fontFamily: "JetBrains Mono,monospace",
      fontSize: 13, letterSpacing: "0.04em", textTransform: "uppercase",
    }}>
      {placeholder.label}
    </div>
  ) : (
    <img src={src} alt="" style={{ width: "100%", height: "100%", objectFit: fit as React.CSSProperties["objectFit"], display: "block" }} />
  );
  return (
    <div style={{
...
export function Stage({
  ...
  background = "#f6f4ef", loop = true,
  autoplay = true, persistKey = "animstage",
  children,
}: {
  width?: number; height?: number; duration?: number;
  background?: string; fps?: number; loop?: boolean;
  autoplay?: boolean; persistKey?: string;
  ...
  return (
    <div ref={stageRef} style={{
      ...
      <PlaybackBar
        time={displayTime} actualTime={time} duration={duration}
        playing={playing}
...
function PlaybackBar({
  ...
  time: number; actualTime?: number; duration: number; playing: boolean;
    ...
    <div style={{
      ...
      <div ref={trackRef} onMouseMove={onTrackMove} onMouseLeave={() => { if (!dragging) onHover(null); }}
        ...
        <div style={{
          ...
          background: "#fff", borderRadius: 6, boxShadow: "0 2px 4px rgba(0,0,0,0.4)",
        }} />
      </div>
